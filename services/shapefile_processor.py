...
class ShapefileProcessor:
        ...
        try:
            ...
            for idx, row in gdf.iterrows():
                try:
                    ...
                    logger.error(
                        f"Erro processando feature {idx}: {feature_error}")
                    # Não interrompe o processamento, apenas pula esta feature
                    continue

            if not processed_features:
                raise ValueError(
                    "Nenhuma feature válida foi processada do shapefile")
            ...
            logger.error(
                f"Erro no processamento do shapefile: {str(e)}", exc_info=True)
            raise
    ...
    async def _read_shapefile(self, temp_dir: Path) -> gpd.GeoDataFrame:
        ...
        logger.info(
            f"Shapefile carregado: {len(gdf)} features, CRS: {gdf.crs}")
        return gdf
    ...
    async def _process_geometry(self, geometry) -> Dict[str, Any]:
        ...
        if not geometry.is_valid:
            logger.warning(
                f"Geometria inválida encontrada: {explain_validity(geometry)}")
            geometry = geometry.buffer(0)
            if not geometry.is_valid:
                raise ValueError(
                    f"Geometria não pôde ser corrigida: {explain_validity(geometry)}")
    ...
    async def _calculate_area_hectares(self, geometry) -> float:
        ...
        try:
            ...
            logger.warning(
                f"Erro no cálculo de área: {e}. Usando aproximação simples.")
