...
from utils.zip_creator import ZipCreator  # Importação adicionada
import logging
import ee
...
async def download_image_from_shapefile(
    ...
    try:
        ...
        for arquivo in arquivos_obrigatorios:
            if arquivo is None:
                raise HTTPException(
                    status_code=400, detail=f"Arquivo {arquivo.filename} é obrigatório.")

        # Salvar todos os arquivos no diretório temporário
        temp_dir = save_uploaded_files(
            [shapefile, shxfile, dbffile, cpgfile, prjfile])
        ...
        if ee_geometry is None:
            raise HTTPException(
                status_code=400, detail="Shapefile inválido ou vazio.")
        ...
        if not 0 <= cloud_pixel_percentage <= 100:
            raise HTTPException(
                status_code=400, detail="O percentual de nuvens deve estar entre 0 e 100.")
        ...
        # Verificar se o resultado contém arquivos
        if not result:
            raise HTTPException(
                status_code=404, detail="Nenhuma imagem encontrada para os parâmetros fornecidos.")
        ...
        return StreamingResponse(
            ...
            headers={
                'Content-Disposition': f'attachment; filename="shapefile_images.zip"'
            }
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Erro ao processar o shapefile: {str(e)}")
    finally:
